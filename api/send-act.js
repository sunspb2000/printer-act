...
<table width="600" cellpadding="0" cellspacing="0" style="background:white;border-radius:12px;overflow:hidden;">
  <tr><td style="background:#1a3a5c;padding:20px 28px;">
    <table width="100%"><tr>
      <td><div style="color:white;font-size:18px;font-weight:700;">Акт Выполненных Работ</div>
          <div style="color:rgba(255,255,255,0.65);font-size:12px;margin-top:3px;">Сформирован автоматически</div></td>
      <td align="right">
        <div style="color:white;font-size:22px;font-weight:700;">№ ${actNum}</div>
        <div style="color:rgba(255,255,255,0.65);font-size:12px;">${dateFormatted}</div>
      </td>
    </tr></table>
  </td></tr>
  <tr><td style="padding:28px;">
    <p style="font-size:15px;color:#333;margin:0 0 12px;">Акт выполненных работ сформирован и прикреплён к письму в виде PDF.</p>
  ...
  const resendRes = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [managerEmail],
      subject: `Акт №${actNum} | ${orgName || '—'} | ${model || '—'} | ${dateFormatted}`,
      html: htmlBody,
      attachments: [{
  ...
  if (!resendRes.ok) {
    ...
    return res.status(500).json({ error: err.message || 'Ошибка Resend', details: err });
  }
